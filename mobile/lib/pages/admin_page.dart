...
class _ListingsTabState extends State<_ListingsTab> {
  ...
  Future<void> _load() async {
    setState(() { _loading = true; _error = null; });
    try {
      final data = await ApiService().getAdminListings(limit: 50);
  ...
  Widget build(BuildContext context) {
    ...
    final listings = (_data?['listings'] as List?) ?? [];
            ...
            Row(children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: () async {
                    try {
                      await ApiService().deleteListing(item['id'] as String);
...
class _UsersTabState extends State<_UsersTab> {
  ...
  Future<void> _search(String query) async {
    setState(() { _loading = true; _error = null; });
    try {
      final data = await ApiService().getAdminUsers(limit: 20);
